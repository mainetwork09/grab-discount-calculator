<template>
  <div>
    <v-card class="mb-5" flat>
      <v-card-title>
        <v-icon class="mr-2">mdi-file</v-icon>รายการ
      </v-card-title>
      <v-simple-table>
        <thead>
          <tr>
          ...
          <tr v-for="item in items" :key="item.time">
            ...
            <td class="text-center">
              <v-btn text icon small color="red" @click.stop="btnDelete(item)">
                <v-icon class="rotate">mdi-account-minus</v-icon>
              </v-btn>
            </td>
          </tr>
        </tbody>
      ...
      <v-card-text class="text-right">
        ทั้งหมด
        <b>{{ items.length }}</b> รายการ
        <br />ราคารวมทั้งหมด
        <b>{{ TotalAmountCalc | NumberFormat }}</b> บาท
        <br />ราคาหลังคิดส่วนลด
        <b>{{ grandTotalCalc | NumberFormat }}</b> บาท
        <br />
      </v-card-text>
    </v-card>
  </div>
  ...
  computed: {
    ...
    grandTotalCalc: function() {
      ...
      items.forEach(element => {
        element.total =
          element.amount -
          (discountFromGrab * element.amount) / this.TotalAmountCalc +
          deliveryFeeFromGrab / items.length;
